...
class Lead_industry extends Security_Controller {
    ...
    function index() {
        return $this->template->view("lead_industry/index");
    }
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Lead_industry_model->get_one($this->request->getPost('id'));
        return $this->template->view('lead_industry/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if (!$id) {
            //get sort value
            $max_sort_value = $this->Lead_industry_model->get_max_sort_value();
            $data["sort"] = $max_sort_value * 1 + 1; //increase sort value
        }

        $save_id = $this->Lead_industry_model->ci_save($data, $id);
    ...
    function update_field_sort_values($id = 0) {
        ...
        if ($sort_values) {
            ...
            foreach ($sort_array as $value) {
                ...
                $this->Lead_industry_model->ci_save($data, $id);
            }
        }
    }
    ...
    function delete() {
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Lead_industry_model->delete($id, true)) {
    ...
    function list_data() {
        $list_data = $this->Lead_industry_model->get_details()->getResult();
        $result = array();
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Lead_industry_model->get_details($options)->getRow();
