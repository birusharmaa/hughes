...
class DispatchAdvice extends Security_Controller
{
    ...
    {  
        $orderStatDrop = '';
        $order_status = $this->Dispatch_status_model->get_details()->getResult();
        foreach ($order_status as $order_statusData) {
            $orderStatDrop .= '<li><a class="dropdown-item orderStatusDropdown" href="#" id ="'.$order_statusData->id.'" data-order-id ="'.$data->id.'">' . $order_statusData->title . '</a></li>';
        }      
        $order_url = "";
        if ($this->login_user->user_type == "staff") {
        ...
        if ($this->login_user->user_type == "staff") {
            if(empty($data->$order_status)){
                ...
                <button class="badge dropdown-toggle" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false" style="background-color:#f1c40f">New Dispatch</button>
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">' . $orderStatDrop . '</ul></div>';
            }
            else{
                $row_data[] = '<div class="dropdown">
                <button class="badge dropdown-toggle" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false" style="background-color:'.$data->dispatch_order_color.'">'.$data->dispatch_order_status_tit.'</button>
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">' . $orderStatDrop . '</ul></div>';
            }
